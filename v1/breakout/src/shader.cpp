...
constexpr std::string VERTEX_TYPE{"VERTEX"};
constexpr std::string FRAGMENT_TYPE{"FRAGMENT"};
constexpr std::string GEOMETRY_TYPE{"GEOMETRY"};
constexpr std::string PROGRAM_TYPE{"PROGRAM"};
...
void Shader::Compile(const char *vertexSource, const char *fragmentSource,
  ...
  checkCompileError(sVertex, VERTEX_TYPE);

  // fragment Shader
  sFragment = glCreateShader(GL_FRAGMENT_SHADER);
  glShaderSource(sFragment, 1, &fragmentSource, nullptr);
  glCompileShader(sFragment);
  checkCompileError(sFragment, FRAGMENT_TYPE);
  ...
  if (geometrySource != nullptr) {
    ...
    checkCompileError(gShader, GEOMETRY_TYPE);
  }
  ...
  checkCompileError(this->ID, PROGRAM_TYPE);
  // delete the shaders as they've link to our program now and no longer
  // necessary
